...
export function articleLink( data, headTag = 'h2' ) {
  ...
  return `
    <article${ data.featured ? ' class="featured"' : '' }>
      <a href="${ data.link }">
        ${ data.imageThumb ? `<img src="${ data.imageThumb }"${ data.imageAlt ? ` alt="${ data.imageAlt }"` : '' } loading="eager">` : '<figure><svg class="spotlight"><use xlink:href="#spotlightlogo"></use></svg></figure>' }
