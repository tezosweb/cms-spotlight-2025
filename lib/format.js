...
export function articleLink( data, headTag = 'h2' ) {
  ...
  return `
    <article>
      <a href="${ data.link }">
        ${ data.imageThumb ? `<img src="${ data.imageThumb }"${ data.imageAlt ? ` alt="${ data.imageAlt }"` : '' }" loading="auto">` : '<figure><svg class="spotlight"><use xlink:href="#spotlightlogo"></use></svg></figure>' }

        <${ headTag }>${ data.title }</${ headTag }>

        ${ data.organization ? `<p class="organization">By ${ data.organization }</p>` : '' }
