...
export function articleLink( data, headTag = 'h2', loading = 'eager' ) {
  ...
  return `
    <article${ data.featured ? ' class="featured"' : '' }>
      <a href="${ data.link }">
        ${ data.imageThumb ? `<img src="${ data.imageThumb }"${ data.imageAlt ? ` alt="${ data.imageAlt }"` : '' } loading="${ loading }">` : '<figure><svg class="spotlight"><use xlink:href="#spotlightlogo"></use></svg></figure>' }
